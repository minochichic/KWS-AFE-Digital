...
| 트랙 | 음량 방어 | 기대 | 실측 낙폭 |
|---|---|---|---|
| `xl_g12` (xlse) | **구조적** 부분 불변 + 증강 | 완만한 낙폭 | **21.2pp** |
| `fx_d0` (fixed) | 없다 (분모가 없다) | 가파를 것 | **50.6pp** |
| `fx_q75` (fixed) | **증강뿐** | 증강이 방어해 줄 것 | **50.0pp** ← 안 줄었다 |

> 세 번째 줄이 예측이 빗나간 자리다. 증강은 낙폭을 0.6pp 줄이는 대신 0 dB
> 정확도를 10.1pp 가져갔다. **상쇄할 분모가 없으면 강건성은 학습으로 만들어지지
> 않는다.** 그래서 트랙 2 의 기준선은 증강 없는 `fx_d0` 다.
...
# ══ 트랙 2 — 다이오드-OR 없는 고정 임계 ═══════════════════════════════════
#   ★ 지금 진행 중인 트랙이다. 다이오드-OR 을 회로로 만들 수 있을지가 아직
#     미지수라, 그게 확정될 때까지 트랙 2 가 실질적인 기준선이다.
#
#   실측 결과 (0 dB test, ROADMAP 게인 스윕):
#     fx_d0   0.7171   증강 없음          <- ★ 트랙 2 최고 기록
#     fx_q75  0.6164   ±12 dB 증강
#     fx_g12  0.5763   q=1.0, ±12 dB      <- 폐기. 태그 재사용 금지
#
#   ⚠️ 예측이 빗나갔다. "정규화가 없으니 게인 증강이 유일한 방어" 라고 보고
#      fx_q75 를 본선으로 잡았는데, 증강은 fixed 에서 순손실이었다 (-10.1pp).
#      낙폭도 못 줄였다 (50.6 -> 50.0pp). 상쇄할 분모가 없으면 강건성은
#      학습으로 만들어지지 않고 깨끗한 정확도만 나간다. 증강이 먹히는 것
#      자체가 다이오드-OR 의 값이다.
# run('fx_d0',   FIXED)                                          # ★ 트랙 2 기준선
# run('fx_q75',  {**FIXED, 'data.aug_gain_db': [-12.0, 12.0]})   # 증강 대조군 (더 낮음)
...
# gain_sweep('xl_g12'); gain_sweep('fx_d0')   # ★ 두 트랙 공통 축 (§6f)
# dead_channels('fx_d0')      # fixed 는 threshold 클램프가 없다 (§6f)"""),
