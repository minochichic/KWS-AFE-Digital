...
# --- cmp 에 내부 풀다운 -- AFE 가 없을 때를 위한 것이다 --------------------- #
# AFE 를 안 물리면 이 16 핀이 뜬다. 뜬 CMOS 입력은 주변 노이즈를 주워 0/1 을
# 왕복하고, 그것이 kws_frame_ctrl 의 **sticky OR** 로 들어간다. 그 래치는
# "10 ms 창 안에 한 번이라도 1 이면 1" 이므로(CLAUDE.md 2.8) 한 번의 글리치가
# 그 창을 통째로 1 로 만든다 -- **노이즈에 가장 취약한 형태의 입력**이다.
#
# 풀다운을 걸면 확정된 0(= -1, "이 대역에 에너지 없음")으로 읽힌다. 그건 실제
# 관측값이고 conv1 이 그것에 대해 학습됐다.
#
