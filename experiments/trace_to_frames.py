...
def write_vectors(frames: List[int], out: Path, want: int) -> None:
    ...
    tb_top compares against a class, so the trace needs one to compare TO. The
    only honest choice is what OUR pipeline says about the same audio: then a
    mismatch is not a broken testbench, it is the measurement -- the circuit's
    bits chose a different word than our simulation's bits did. That is the
    whole question a real trace exists to answer.
    """
    out.mkdir(parents=True, exist_ok=True)
    (out / "input.hex").write_text(to_hex(frames))
    (out / "predictions_fixed.txt").write_text(f"{want}\n")
    (out / "paths.vh").write_text("\n".join([
        "// GENERATED by experiments/trace_to_frames.py -- do not edit.",
        "// A comparator trace from the analog side, as tb_top vectors.",
        "//",
        "// tb_top ONLY. The per-layer golden vectors are not here: they",
        "// describe our simulation's bits, and these are someone else's. A",
        "// different testbench will fail on an undefined macro, which is the",
        "// loud version of the right answer.",
        "`ifndef KWS_GOLD_VH", "`define KWS_GOLD_VH",
        "`define KWS_GOLD_CLIPS   1",
        f'`define KWS_GOLD_INPUT             "{out.as_posix()}/input.hex"',
        f'`define KWS_GOLD_PREDICTIONS_FIXED "{out.as_posix()}'
...
def main() -> None:
    ...
    if args.compare:
        ...
        ours_class = _compare(frames, args.compare, args.wav)

    if args.vectors:
        if ours_class is None:
            raise SystemExit("--vectors needs --compare and --wav: tb_top "
                             "compares against a class, and the only honest "
                             "one is what our pipeline says about the same "
                             "audio")
        write_vectors(frames, args.vectors, ours_class)
        print(f"\nwrote {args.vectors}/ -- run it with")
        print(f"  ./rtl/run_tb.sh top xl_g12 {args.vectors}")
...
def _compare(theirs: List[int], run: Path, wav: Path) -> int:
    """Our AFE on the same clip, bit for bit against theirs. Returns our class.
    ...
    with torch.no_grad():
        xb = afe(w[:1], target_T=cfg.model.T)         # [1, 16, 128] in +-1
        ours_class = int(model(xb).argmax(1)[0])
        x = xb[0]
    ...
    print(f"\n  our pipeline calls this clip: {class_names()[ours_class]} "
          f"({ours_class})")
    return ours_class
