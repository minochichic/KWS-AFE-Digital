...
def write_paths_vh(man: Dict[str, Any], out: Path, gen_dir: str) -> None:
    """Every ROM's path as a macro, so a testbench names files instead of paths.

    Verilog-2005 cannot build a string from a macro and a literal, so the whole
    path has to be the macro. That is what lets one testbench run against any
    export: switching tags changes which paths.vh is included, and nothing else.

    The alternative was copying the RTL per track, which would mean an analog
    change costs an RTL edit -- the one thing docs/ICD.md exists to prevent.
    """
    L = [f"// GENERATED by export/emit.py -- do not edit.",
         f"// ROM paths for tag {man['tag']}, relative to the repo root.",
         "`ifndef KWS_PATHS_VH", "`define KWS_PATHS_VH",
         f'`define KWS_TAG          "{man["tag"]}"',
         f'`define KWS_GEN_DIR      "{gen_dir}"', ""]
    for name, rom in sorted(man["roms"].items()):
        L.append(f'`define KWS_ROM_{name.upper():<16} "{gen_dir}/{rom["file"]}"')
    L += ["", "`endif"]
...
def main() -> None:
    ...
    write_paths_vh(man, out, args.out or str(run / "rtl"))
