...
def dump_golden(model: BinaryMatchboxNet, x: torch.Tensor, out: Path,
                tag: str, source_split: Optional[str] = None,
                write_layers: bool = True) -> Dict[str, Any]:
...
def write_golden_paths_vh(man: Dict[str, Any], out: Path, gold_dir: str) -> None:
    """The same trick as export/emit.write_paths_vh, for the golden vectors."""
    L = ["// GENERATED by export/golden.py -- do not edit.",
         f"// Golden vector paths for tag {man['tag']}, from the repo root.",
         "`ifndef KWS_GOLD_VH", "`define KWS_GOLD_VH",
         f"`define KWS_GOLD_CLIPS   {man['n_clips']}",
         f'`define KWS_GOLD_DIR     "{gold_dir}"', ""]
    for key, meta in sorted(man["files"].items()):
        L.append(f'`define KWS_GOLD_{key.upper():<18} "{gold_dir}/{meta["file"]}"')
    # the loose files that have no entry in `files`. Skipping the ones that do
    # matters: a repeated `define is a redefinition warning, and a warning in a
    # generated header is one nobody will read twice.
    named = {k.upper() for k in man["files"]}
    for extra in ("labels.txt", "logits.txt", "predictions.txt",
                  "predictions_fixed.txt", "pooled.txt"):
        n = extra.rsplit(".", 1)[0].upper()
        if n not in named:
            L.append(f'`define KWS_GOLD_{n:<18} "{gold_dir}/{extra}"')
    L += ["", "`endif"]
...
def main() -> None:
    ...
    if int(x.shape[0]) < args.clips:
        raise SystemExit(
            f"asked for {args.clips} clips, the {args.split} split gave "
            f"{int(x.shape[0])}. Lower --clips, or try --split test -- a short "
            f"dump that reports itself as long is the failure this guard "
            f"exists to prevent.")
