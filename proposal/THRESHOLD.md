...
산포 = `(R5/R4) × V_OS + 비교기 오프셋`이며, OPA379의 V_OS는 typ 0.4 mV / **max 1.5 mV**
(데이터시트 SBOS347D)다. 현재 이득 4.7배에서:

| V_OS | 산포 | 마진/산포 |
|---|---|---|
| typ 0.4 mV | 2.9 mV | **1.39배** ✅ |
| max 1.5 mV | 8.1 mV | **0.50배** ⚠️ |

**검출기 이득을 올리면 산포도 같이 커지는데 LSE 바닥은 안 커진다.** 그래서 이득은
검출기가 아니라 **프리앰프**에서 뽑아야 한다 (프리앰프는 필터 앞이라 정지점을 안 건드린다).
자세한 내용과 시뮬 근거는 [`../analog/AFE/SPICE_FINDINGS.md`](../analog/AFE/SPICE_FINDINGS.md) §7.
