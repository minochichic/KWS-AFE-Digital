...
$$V_{ref} = \underbrace{917.8\ \text{mV}}_{\text{검출기 정지점 (실측)}} + \;\underbrace{\delta}_{= \;0}$$

**왜 필요한가**: 무음일 때는 16채널이 전부 잡음뿐이다. 그래도 그중 하나는 최댓값이라
비율이 1에 가까워지고, **무음인데 발화**할 위험이 있다.

**δ = 0이다.** 학습에서 그렇게 확정했고(`xmax_floor_frac: 0.0`), 16채널 넷리스트로
확인했다. 무음 마진을 만드는 것이 δ가 아니기 때문이다 — 다이오드-OR가 만드는
**LSE 바닥** $T\ln 16$이 만든다:

$$\text{마진}_c \;=\; \alpha_c \times (T \ln 16) \;=\; \alpha_c \times 70\ \text{mV}$$

무음에서 $V_{max}$는 정지점보다 70 mV 위에 떠 있으므로, 분압이 $V_{ref}$에서 그만큼
올라간 자리에 임계를 놓는다. δ가 0이어도 16채널 전부 조용하다 (시뮬 확인).
...
이 4.0 mV를 갉아먹는 것이 **채널별 정지점 산포**이고, 이게 이 설계 전체의 발목이다.
산포 = `(R5/R4) × V_OS,opamp + V_OS,비교기` 다. 두 항의 출처가 다르다:

| | typ | max | 이득이 곱해지나 |
|---|---|---|---|
| OPA379 (SBOS347D) | 0.4 mV | 1.5 mV | **예** — (R5/R4)배 |
| **LPV7215 (SNOS977, 1.8 V)** | 0.4 mV | **6 mV** (온도 8 mV) | **아니오** |
...
> ⚠️ **worst case 에서는 비교기 오프셋 6 mV 하나가 마진 4.0 mV 보다 크다.** 검출기
> 이득으로는 풀리지 않는다 — 이득을 낮추면 op-amp 항만 줄고 비교기 항은 그대로인데,
> 신호가 같이 줄어 교차가 완만해져서 오히려 손해다.
> 그림: [`../docs/diagrams/18_head.svg`](../docs/diagrams/18_head.svg)

**제작 시 확인 요청**: 오프셋이 더 작거나 **히스테리시스 내장**인 비교기가 있으면 그쪽이
낫다. LPV7215의 typ 0.4 / max 6 mV 는 15배 격차이고, 이 항이 지금 설계의 병목이다.

> **만드는 법**: 1.8 V 레일에서 저항 분압 → **유니티 게인 버퍼(OPA379)** → 16개 Ra의
> 공통단. 버퍼는 선택이 아니라 필수다 (아래).
>
> **더미 검출기(추종형)는 쓰지 않는다.** 처음에 그걸 권했으나 시뮬에서 안 되는 것으로
> 확인됐다. 이유는 [`../analog/AFE/SPICE_FINDINGS.md`](../analog/AFE/SPICE_FINDINGS.md) §4:
> 정지점은 `da + (R5/R4)·(vf − da)`, 즉 **그 채널 op-amp 오프셋의 4.7배**인데 이건
> 소자마다 랜덤이라 수동 회로로는 추종 대상 자체가 없다.
...
- [ ] `V_ref`는 **버퍼를 거친 저항 분압**으로 만들고, 브링업에서 트림한다.
      더미 검출기 추종형은 **쓰지 않는다** (§D — 추종할 대상이 소자별 랜덤이다)
- [ ] **`V_ref` 버퍼는 생략 불가.** 정지점에서 D2가 역바이어스라 엔벨로프단이 고임피던스다.
      버퍼 없이 16개 분압을 물리면 기준이 **40 mV 밀린다** (시뮬 확인)
- [ ] OR 다이오드는 검출기와 **같은 BAT54**를 쓰고, 강하 보상용 짝은 **같은 패키지**에서 뽑는다
- [ ] 정전류 싱크는 **1 µA**. 반드시 *정전류*여야 한다 — 저항으로 대신하면 전류합이
      $V_{or}$를 따라가서 log-sum-exp가 성립하지 않는다. 100 nA로 내리면 다이오드
      $I_s$에 가까워져 LSE 오차가 10배가 된다
...
- [ ] 보드가 나오면 **채널별** v_env 정지점을 잰다 (설계값 917.8 mV). 채널 간 산포가
      4 mV를 넘으면 최악 채널의 무음 마진을 먹으므로 δ를 올려야 한다 (§D)
