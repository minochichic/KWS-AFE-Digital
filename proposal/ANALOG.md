...
| 비교기 | LPV7215, 0.58 µA — **채널당 2개** (§3-5) | +2.5pp @ +17 µW |
...
| 채널별 분압 | 저항 ×64 (채널당 2쌍) | $\alpha_{c,i} = R_b/(R_a+R_b)$ |
| 비교기 추가 | LPV7215 ×16 (채널당 1개 → 2개) | 2비트 온도계 코드 |
...
| 추가: 새 분압 ×32 (V_max↔V_ref, ~0.1 V) | +6 µW |
| 절감: 기존 분압 ×16 (1.8 V 전체) | **−52 µW** |
| **순증** | **≈ −13 µW** |

**비교기를 2배로 늘리고도 총 전력은 줄어든다.** 분압이 1.8 V 전체가 아니라
`V_max − V_ref`(~0.1 V)에만 걸리게 되면서 절감분이 그보다 크기 때문이다.

### 3-5. 왜 채널당 비교기 2개인가

정확도 사다리가 **양자화에 8.4pp**를 귀속시켰다(연속 0.862 → 1비트 0.778). 같은
엔벨로프를 두 개의 분압으로 나눠 두 비교기가 보면 **2비트 온도계 코드**가 되고:
...
**+2.5pp를 +17 µW에** 샀다. 학습된 두 임계의 간격은 0.0513~0.5346으로 **붙은 쌍이
없으므로** 두 번째 비교기가 실제로 정보를 싣고 있다.

α는 채널당 2개(총 32개)가 되고, 분압도 채널당 2쌍이 된다. 나머지는 그대로다.
