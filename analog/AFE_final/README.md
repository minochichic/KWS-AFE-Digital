...
## 4. 결과 (E1, E2) — 두 가설 모두 반증됨

### E1. 캘리브레이션 재평가 — 캘리브레이션만으론 부족

오프셋-인지 학습 모델(vos=0.035 학습)을 vos를 바꿔 평가:

| vos | 0.000 | 0.005 | 0.010 | 0.020 | 0.035 |
|---|---:|---:|---:|---:|---:|
| test acc | 0.7432 | **0.7471** | 0.7437 | 0.7456 | 0.7226 |

- **vos 0~0.02에서 완전히 평평(~0.744)** → 강건 학습이 오프셋 민감도를 실질적으로 제거.
- **그러나 캘리브레이션 이득은 +2pp뿐**(0.723→0.744). 예상했던 0.78~0.80은 **오지 않음**.
- **이유**: 이 모델은 **vos=0에서도 0.743** — 오프셋-free 모델(0.802)보다 **6pp 낮다.**
  즉 오프셋-인지 학습이 **깨끗한 정확도 6pp를 강건성과 교환**했다(robustness tax).
  캘리브레이션으로 오프셋을 없애도 그 tax는 환불되지 않는다.

### E2. 고정 스케일(절대 임계) — per-clip AGC가 ~12pp 가치

`normalize="fixed"` (= Cerutti 고정 min-max = 절대 임계):

| 정규화 | val | 비고 |
|---|---:|---|
| per-clip min-max (기존) | **0.82** (test 0.802) | 비인과 AGC, 하드웨어 대응물 없음 |
| **fixed (절대 임계)** | **~0.70** (train 0.705) | 고정 R7/R8로 실현 가능 |

(test acc 미기록 — 확인 필요)

- **약 −12pp.** 절대 임계는 발화별 음량에 적응하지 못해(조용한 발화 전부 −1, 큰 발화 포화)
  정보를 잃는다.
- **⚠️ 이전 결론 반전**: "Cerutti가 고정 스케일을 썼으니 AGC 불필요"라고 했으나,
  데이터는 **per-clip AGC가 ~12pp 가치**임을 보인다. → **아날로그 AGC 제안이 다시 유효**하다.
  Cerutti는 이 비용을 지불하고 **64채널로 보상**했을 가능성(8ch 76% / 64ch 86%).
- 잔여 의문: fixed_hi를 batch max로 잡아 이상치에 민감할 수 있음(고분위수 사용이 더 나을지).
  단 이진 결정은 절대임계와 수학적으로 동일하므로 −12pp의 본질은 **음량 불변성 상실**로 판단.

### 종합: 정확도 위계 (spice+√ 16ch)

| 구성 | 정확도 | 하드웨어 실현성 |
|---|---:|---|
| per-clip AGC + 무오프셋 | 0.802 | ❌ (이상화) |
| per-clip AGC + 오프셋-인지 | 0.744 | ⚠️ AGC 필요 |
| 절대임계 + 무오프셋 | ~0.70 | ✅ 고정 R7/R8 |
| 절대임계 + 오프셋(예정 E3) | ? | ✅ 가장 충실 |

→ **하드웨어 충실도를 높일수록 정확도가 내려간다**는 것이 이 프로젝트의 핵심 트레이드오프이며,
이제 각 단계의 비용이 정량화되었다: **AGC 포기 −12pp, 오프셋 강건성 −6pp.**

### 다음 (가장 값싼 결정적 실험)
오프셋-**free** 모델을 작은 vos에서 평가 → "잘 캘리브레이션" vs "강건 학습" 직접 비교.
0.005에서 0.744를 넘으면 캘리브레이션 전략이 우월(tax 없음), 못 넘으면 강건 학습이 정답.
