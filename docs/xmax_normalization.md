...
| **`xmax` floor=0.05** | 0.802 | **0.781** | 정밀 max | ✅ 없음 |
| `xmax` floor=0.02 | 0.808 | 0.776 ⚠️ | 정밀 max | ⚠️ §4-3 퇴화 |
| `xmax` floor=0.10 | 0.795 | 0.756 | 정밀 max | ✅ 없음 |
| `fixed` q=0.75 | — | 0.726 | 없음 | — |

**`floor=0.05`가 최종 확정값**이다. §4-3의 퇴화가 없으면서 `floor=0.02`(0.776)보다도
**높다** — 그 2pp는 애초에 실익이 아니었다.

**AGC와 0.8pp 차이**이므로 피드백 루프를 감수할 이유가 없다. **AGC는 폐기한다.**

### 6-1. 오프셋 강건성 (floor=0.05)
...
### 6-2. 손실의 완전 분해 (`binarize=False`)

비교기만 제거하고 연속 엔벨로프를 넣어 측정했다. 세 런 모두 **네트워크 내부는 이진 그대로**다.

| 입력 | test |
|---|---:|
| 64채널 **연속** log-mel + minmax | **0.9198** |
| ↓ **−5.8pp** — 프론트엔드 (64→16ch, mel→spice, log→√, minmax→xmax) | |
| 16채널 **연속** spice+√+xmax(0.05) | **0.8617** |
| ↓ **−8.0pp** — **비교기 (1비트화)** | |
| 16채널 **이진** = 현재 실제 모델 | **0.7813** |

두 가지가 확정된다:

1. **모델은 무죄** — 이진 네트워크가 좋은 입력에서 0.9198을 낸다.
2. **연속값 16채널 천장이 0.8617** — 정규화를 완벽하게 만들어도(minmax 0.802) 85%에
   못 미치므로 **남은 격차를 정규화에서 짜낼 수 없다.**

그리고 우리 78.1%은 Cerutti의 채널 수 곡선(8ch 76.3% → 64ch 86.0%, 2배당 +3.2pp)이
예측하는 16ch 값 79.5%에서 **1.4pp** 안쪽이다 → **16채널이 부족한 것이지 우리가
틀린 게 아니다.** 자세한 분석은 [`experiments_log.md`](experiments_log.md) Stage 3.
...
afe:
  ...
  normalize: xmax
  xmax_floor_frac: 0.05      # 확정값. 프레임 분위수, sqrt 가드(1e-3) 위여야 함
```
...
1. ~~`floor_frac=0.05` 재학습~~ ✅ 완료 — **0.781 확정**
2. **채널당 비교기 2개 (2비트)** — §6-2가 비교기에 −8pp를 귀속시켰고, +17 µW로 그중
   일부를 회수할 수 있다. **현재 최우선 레버.**
3. **하드웨어 정합 형태로 재학습** (§5-3) — `α·S + (1−α)·δ` 선형 혼합
4. δ를 실제 mV로 환산 → 동료에게 전달할 `V_ref` 설계값
5. SPICE 역검증 — 학습된 α를 실제 회로에 넣고 나온 이진 이미지로 재평가
