...
def report(doc):
    ...
    h(doc, "1. 설계 요약", 12)
    summary_box(doc, [
        "기능: 아날로그 filter bank·envelope detector·comparator로 ADC·FFT 없이 16채널 binary feature를 추출하고, 이를 12개 클래스"
        "(키워드 10개 + silence + unknown)로 분류하는 always-on keyword spotting(KWS) 시스템",
        "아날로그 전단: 16채널 특징 추출 회로 설계, PCB 설계 및 SPICE 시뮬레이션 완료",
        "구조: partially binarized MatchboxNet(1D TCS, 96.5 k parameters) + multiplier-free integer-only folded RTL + 100 ms "
        "sliding window·consecutive voting",
        f"정확도: Google Speech Commands v2 12-class test {T1B['n_clips']:,}개, integer path {R.pct(T1P['fixed_acc'])} % "
        "(float 대비 quantization loss 없음)",
        f"구현: Xilinx Spartan-7 XC7S75, 50 MHz, WNS {TS['wns']:+.3f} ns, LUT 43 %, inference 57.5 ms/clip(decision hop 100 ms)",
        f"하드웨어 효율: binary layer multiplier 0개(DSP 7개는 tail만), weight memory float32 대비 약 {fp_bits / mem_bits:.1f}배 축소",
        "검증: on-chip self-test로 2,400개 클립–모델 조합(반복 포함 5,400회)에서 소프트웨어와 100 % 일치",
        "응용: 배터리 기반 IoT·웨어러블·스마트홈 기기의 음성 웨이크업, 초저전력 음성 인터페이스(ADC·FFT 제거)",
    ])

    h(doc, "1) 창의성", 11)
    R.bullets(doc, [
        "**Analog binary feature + partially binarized MatchboxNet.** 기존 연구 [1]의 2D CNN BNN 대신 1D time-channel "
        "separable 구조를 채택하고, first layer(INT8)·last layer(fixed-point)를 제외한 중간 block만 binarization하여 곱셈을 "
        "XNOR-popcount로, BN을 integer comparison 하나로 대체하였다.",
        "**Fabrication-aware training.** PCB로 설계된 filter bank의 SPICE response와, resistor divider 하나로 구현되는 "
        "채널별 absolute threshold를 네트워크와 함께 학습(end-to-end)하였다. 좁은 STE window로 binary-input 모델 정확도를 +10.0 %p 개선하였다.",
        "**Trigger-based에서 continuous decision으로.** [1]의 '첫 이벤트 후 1초 1회 분류'를 100 ms 간격 sliding window와 "
        "consecutive voting으로 바꾸고, window 경계에 걸린 단어를 학습하는 partial-window fine-tuning을 도입하였다.",
        "**On-chip self-test.** clip ROM·expected-class ROM·scorer를 칩에 두고 JTAG VIO로 결과만 읽는 구조로, 핀 2개만으로 "
        "수백 클립을 분 단위로 검증한다.",
    ], 10)
    h(doc, "2) 난이도", 11)
    R.bullets(doc, [
        "학습(PyTorch) → integer conversion(export) → RTL → FPGA 구현 → 칩 검증까지 전 과정을 직접 설계하였으며, 소프트웨어 "
        "integer path와 **bit-exact**하게 일치해야 하는 허용 오차 0의 목표를 설정하였다.",
        "BN folding, integer residual addition, Q*.6 fixed-point tail을 chained quantization하여 **accuracy loss 없는** integer-only 경로를 만들었다"
        f"(integer {R.pct(T1B['fixed_acc'])} % vs float {R.pct(T1B['float_acc'])} %).",
        "binary input·16채널이라는 정보 제약 아래에서 표준 12-class 전체 test 세트로 평가하였고, window 경계·continuous decision 조건의 "
        "always-on 성능까지 별도로 측정하였다.",
    ], 10)
    h(doc, "3) 완성도", 11)
    R.bullets(doc, [
        "module level(frame capture, sliding window, voting, plane buffer, TCS block, tail)부터 전체 네트워크까지 golden vector simulation에서 "
        "0 failures, 두 모델 빌드 모두 timing violation 0으로 구현을 완료하였다.",
        "칩에서 2개 클래스 편중 세트와 12개 클래스 균형 세트를 각각 두 모델로 실행하여 모든 실행에서 600/600 일치, "
        "전원 재투입 후 반복 실행도 동일함을 확인하였다.",
        "weight를 ROM 파일로 분리하여 모델을 바꿔도 RTL 수정이 없으며, 학습·export·빌드·검증·보고서 생성 전 과정이 "
        "스크립트로 재현 가능하다.",
    ], 10)
    ...
    R.figure(doc, R.IMG / "fig_system.png",
             "전체 구성. 아날로그 front end가 comparator 출력 16가닥을 내고, FPGA가 10 ms frame capture → 1초 sliding window → "
             "folded BinaryMatchboxNet → consecutive voting을 수행한다.", 16.5)
    h(doc, "2.1 아날로그 특징 추출 전단", 10.5)
    R.para(doc, "[아날로그 전단 구성 및 동작 — 공동 연구자 작성 예정]", italic=True)
    h(doc, "2.2 디지털 분류 가속기", 10.5)
    R.para(doc,
           "아날로그 front end와의 interface는 comparator 출력 16가닥뿐이다. "
           "디지털 단의 동작은 다음과 같다. (1) Frame capture: 10 ms 동안 한 번이라도 1이 된 채널을 1로 기록해 16비트 "
           "프레임을 만든다. (2) sliding window: 최근 100프레임을 circular buffer에 유지하고 100 ms마다 snapshot을 떠서 좌우 "
           "14프레임을 채운 128프레임 입력을 만든다. (3) Network: layer마다 MAC engine 하나를 time-multiplexing으로 재사용하는 folded "
           "구조로 57 ms 안에 12개 클래스 점수를 계산한다. (4) Decision: 같은 keyword가 quiet 대비 margin 이상으로 5회 연속 "
           "나오면 검출하고 1초간 재검출을 막는다. (1), (2), (4)의 continuous decision block은 RTL로 설계하여 XSim module-level simulation"
           "(frame capture 3 frames, sliding window 33 frames·overrun, voting 3 scenarios, 모두 0 failures)으로 검증하였고, "
           "chip-level verification은 (3) network를 대상으로 수행하였다.")
    ...
    R.para(doc,
           "binary layer의 곱셈은 XNOR-popcount로, BN과 sign function은 integer comparison 하나로 바뀌며, 입력이 ±1이므로 conv1도 signed "
           "accumulation만으로 계산된다. 곱셈은 BN이 흡수되지 않는 tail에만 남는다(Table II).")
    ...
    R.figure(doc, R.IMG / "fig_selftest.png",
             "On-chip self-test 구조. clip과 expected class를 BRAM에 저장하고 칩 안에서 모든 분류를 scoring하며, counter만 JTAG VIO로 읽는다.",
             8.5)

    # ---- 3. 설계 과정 및 실험 결과 ----
    page_break(doc)
    h(doc, "3. 설계 과정 및 실험 결과", 12)
    h(doc, "3.1 아날로그 front end 설계 및 시뮬레이션", 11)
    R.para(doc, "[회로·PCB 설계 및 SPICE 시뮬레이션 결과 — 공동 연구자 작성 예정]", italic=True)
    h(doc, "3.2 디지털 단 설계 과정", 11)
    R.para(doc,
           "(1) Training: 아날로그 front end의 SPICE filter response를 적용한 16채널 binary input으로 QAT 학습하고, comparator threshold 16개를 "
           "STE로 함께 학습하였다(Table III). (2) Integer conversion: BN을 integer threshold로 folding하고 tail을 Q*.6 fixed-point로 바꿔 "
           "ROM 파일과 per-layer golden vector를 생성하였다. (3) RTL verification: module별로 golden vector와 bit-exact 비교 후 synthesis·place-and-route하였다. "
           "(4) Chip-level verification: self-test bitstream으로 clip set을 칩에서 실행하였다.")
    ...
    R.para(doc,
           "Critical path는 conv2 depthwise(k=29)의 MAC 입력 선택부터 accumulator까지이며, 가장 느린 speed grade(-1)에서 5 ns 이상 "
           "여유가 있다. 전력은 Vivado 추정으로 총 0.183 W(static 0.094 W, network dynamic 0.058 W)이다.")
    side_by_side(doc,
                 [(R.HW / "device_place.png", 4.6), (R.IMG / "schematic_net_wrapper.png", 11.4)],
                 "(a) XC7S75 placement(Vivado device view). 색칠된 영역은 network block, self-test harness, debug logic이다. "
                 "(b) Post-synthesis schematic: self-test top(kws_selftest_top) 안의 network(kws_top)와 I/O interface.")

    h(doc, "3.4 Chip-level verification 결과", 11)
    ...
    R.para(doc,
           "모든 실행에서 칩과 소프트웨어 integer path가 600개 전부 일치하였다(mismatch rate 95 % upper bound 약 0.13 %).")
    ...
    R.figure(doc, R.IMG / "fig_confusion_bd_base.png",
             "Baseline integer path confusion matrix(clip 수, n = 4,888). 클래스당 396–425개로 [1]의 confusion matrix와 같은 규모.", 8.5)
    R.para(doc,
           "주요 confusion pair는 no↔go, down↔go로 [1]과 같은 경향이다. 연속 동작 인식 성능은 소프트웨어 시뮬레이션으로 평가하였다(validation set, silence–keyword–silence "
           "3초 stream, 100 ms hop, 5-consecutive voting). 그 결과 Partial-75는 keyword recall 69.53 %(Baseline 67.73 %), quiet false alarm "
    ...
    R.para(doc,
           "16채널 정확도(82.6 %)는 [1]의 8채널(76.3 %)과 64채널(86.0 %) 사이이며, 제작 가능한 filter bank·threshold를 전제로 한 "
           "integer path 결과가 칩에서 그대로 재현된다. 다만 FPGA는 static power가 커서 µW급 ASIC과 전력을 직접 비교할 수는 없다.")
