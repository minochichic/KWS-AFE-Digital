...
> ⚠️ **정정 (2026-09-14).** 이 절이 처음에 `W = 1280 ms` 로 계산했는데 **틀렸다.**
> 1280 은 텐서 폭이고, 그 중 좌우 14 프레임은 **항상 `−1` 인 패딩**이라 소리를
> 담지 않는다. 스트림 위를 미는 창이 실제로 덮는 음성은 **100 프레임 = 1000 ms** 다
> (`experiments/eval_streaming.py` 도 `[0,100)`, `[10,110)` … 로 그렇게 만든다).
> 아래 숫자는 전부 `W = 1000` 으로 다시 계산한 것이다.

폭 `W` = **1000 ms** 인 창이 길이 `L` 인 발화를 **온전히** 담으려면 창 시작점이
...
| 발화 길이 `L` | `S = 1000−L` | **10 Hz** 보장 | 5 Hz 보장 |
|---|---|---|---|
| 300 ms | 700 ms | **7** | 3 |
| 400 ms | 600 ms | **6** | 3 |
| 500 ms | 500 ms | **5** | 2 |
| 600 ms | 400 ms | **4** | 2 |
| 700 ms | 300 ms | **3** ← N=3 한계 | 1 |
| 800 ms | 200 ms | 2 | 1 |
| 900 ms | 100 ms | 1 | 0 |

**N 은 자유 하이퍼파라미터가 아니라 기하로 상한이 걸린다:**

```
N 연속이 애초에 가능하려면   L <= W - N*P
  10 Hz, N=3  ->  L <= 700 ms
  10 Hz, N=5  ->  L <= 500 ms      <- GSC 발화 상당수가 여기 안 든다
   5 Hz, N=3  ->  L <= 700 ms 이지만 보장 3개가 L<=700 에서만 나온다
```

> ⚠️ **이 상한은 "온전히 담는 창" 의 하한일 뿐, 실제 표의 개수가 아니다.**
> 한때 이 자리에 "그래서 `diagnose_streaming` 의 257건(5연속 부족)이 설명된다" 고
> 적었는데 **데이터가 그걸 반증한다.** 같은 trace 에서
>
> | | recall | quiet FA |
> |---|---:|---:|
> | N=4, margin 0 | 75.31% | 101/256 |
> | N=5, margin 0 | 73.59% | 63/256 |
>
> 표를 하나 더 요구하는데 **−1.72pp** 밖에 안 떨어진다. 기하가 묶고 있었다면
> (GSC 발화 상당수가 500 ms 를 넘으므로) 훨씬 크게 무너졌어야 한다. 즉
> **부분적으로만 겹친 창도 자주 맞는다** — 모델이 이 하한을 이미 넘어선다.
>
> 그리고 N 은 이미 스윕됐다. **같은 오검출 예산(28/256)에서 N=4 가 69.84%,
> N=5 가 70.00% 로 사실상 동률**이고, N 을 내리면 recall 이 오르는 만큼 margin 으로
> 되갚는다. **"N 을 내리면 공짜로 얻는다" 는 것도 틀렸다.**

그러므로 이 표는 **왜 실패하는가의 설명이 아니라, 어떤 N 이 애초에 말이 되는지의
경계선**으로만 쓴다. `L ≤ W − N·P` 를 넘는 N 은 부분 겹침 창의 성능에 전적으로
기대는 설정이라는 것만 기억하면 된다.

### 그래서 257건의 원인은 아직 안 갈렸다

`correct_but_not_consecutive` 257건은 **두 가지가 섞여 있다:**

| | 뜻 | 답 |
|---|---|---|
| 정답 창이 **N개 미만** | 표 자체가 부족 | 창을 더 만들거나 부분 겹침 창 성능을 올린다 |
| 정답 창은 **N개 이상인데 안 붙어 있다** | 인접 창 사이에서 흔들린다 | 점수 평활 또는 안정성 학습 |

**둘은 처방이 정반대인데 지금 한 범주에 들어 있다.** 가르는 측정은 공짜다 —
기존 window CSV 에서 사례마다

- `n_correct` : 정답 top-1 창의 **총 개수**
- `longest_run` : 정답 top-1 이 **연속된 최장 길이**

를 세어 두 분포를 보면 된다. GPU 도 체크포인트도 필요 없다. **이것을 재기 전에
적응 학습을 설계하면 무엇을 고치는지 모르는 채로 학습하게 된다.**
