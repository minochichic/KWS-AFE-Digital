...
### Phase B 진행 노트 (1차 재학습 디버깅)

첫 spice 재학습(separable)이 **test 0.586**(mel dense 0.8715 대비 급락). 로컬 GSC로
프론트엔드를 진단해 원인 두 개를 잡음:

1. **`|H|²` 버그**: 파워 스펙트럼엔 `|H(f)|²`를 곱해야 하는데 `|H|`를 곱했음
   (`data/afe.py` `_bands`에서 수정). 필터가 √만큼 넓어져 채널 겹침 악화.
2. **log 압축이 주범**: 회로는 log를 안 함(검출기는 진폭≈√power). log가 2차 GIC의
   넓은 스커트 꼬리를 끌어올려 **채널 간 상관 0.89**(mel 0.75)로 과중복 → 정보 손실.
   → **`compression: "sqrt"`**(회로 충실) 추가. 실측 채널 상관:

   | 프론트엔드 | 채널 상관 | 활동(init 후) |
   |---|---:|---|
   | mel + log (baseline) | 0.75 | 0.28–0.36 |
   | spice + log | 0.89 | 0.33–0.36 (중복) |
   | **spice + sqrt** | **0.56** | 0.17–0.22 (정상, mel보다 선택적) |

`init_thresholds` 후 spice+sqrt는 죽은 채널 0 → 재학습 시 회복 기대. 실행:
`{'afe.filterbank_source':'spice', 'afe.compression':'sqrt'}`.
(sqrt는 검출기 압축의 √부분만 반영. 데드존/포화는 아직 미반영 — stage 2 후보.)
