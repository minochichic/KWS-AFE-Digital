# AGENTS.md — BinaryMatchboxNet KWS 프로젝트

이 파일은 Codex가 매 세션 시작 시 자동으로 읽는 프로젝트 헌장이다.
...
**결정 기록 (2026-08-26, 사용자 승인):** 소프트웨어 모델은 **`normalize: fixed`
(트랙 2, 절대 임계)** 로 간다. *"학습된 임계값을 이용해 저항비를 고정하여 PCB 를
만든다."* 종전 §0 은 `xlse` + `aug_gain_db=[-12,+12]` 였고 정확도는 그쪽이 높지만
(0.8445 vs 0.817), `xlse` 의 임계값은 채널 고유값이 아니라 **16채널 공통 노드 기준의
분압**이라 다이오드-OR 회로가 실제로 만들어져야 성립한다. `fixed` 는 lo/hi 가
데이터셋 상수여서 `env >= thr` 이 그대로 절대 임계이고 **채널마다 R7/R8 분압 하나**로
떨어진다. 게인 증강은 `fixed` 아래서 −10.1pp 이므로 **같이 켜지 않는다**(고정 임계
아래서 게인은 교란이 아니라 이미지 삭제다 — `EXPERIMENT_MAP.md`).
트랙 1 은 폐기가 아니라 보류다: 동료 회로가 다이오드-OR 을 해주면 다시 연다.
...
- 새 파일을 만들거나 주요 설계를 정할 때는 **왜 그렇게 했는지 짧게 설명**하고,
  이 AGENTS.md의 규칙과 충돌하면 진행 전에 질문.
