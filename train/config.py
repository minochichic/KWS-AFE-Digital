...
class AFEConfig:
    ...
    #
    # ⚠️ 이것은 **재매개변수화이지 수정이 아니다** (2026-08-30 확인).
    # env_c 를 k_c 배 하고 thr_c 를 같이 옮기면 이진 출력이 비트 단위로 같다 --
    # 실측으로 일치율 1.000000. thr 은 자유 파라미터이므로 학습이 언제든 보상할 수
    # 있고, 따라서 **도달 가능한 정확도도, 최적점의 물리적 강건성도 바뀌지 않는다.**
    # 최적 Vthr 은 물리적 지점이고 좌표계를 바꾼다고 움직이지 않는다.
    #
    # 그러면 무엇이 바뀌나: 초기값이 앉는 자리와 gradient 스케일이다. SGD 는 재매개
    # 변수화에 불변이 아니므로 다른 국소해에 착지할 수 있다 -- 원리가 아니라 운이다.
    # 그리고 정규화 도메인이 전압에 비례해져서 물리 오차 주입이 채널별 환산 없이
    # 맞는다. 다만 swing_sensitivity 가 이미 그 환산을 하고 있으므로 측정은 원래
    # 옳았다. **편의이지 해결책이 아니다.**
    #
    ...
    # 임계값 하한 (정규화 단위). 0.0 = 끔 = 정확한 no-op.
    #
    # 왜 있나: 물리 오차 1 mV 가 뒤집는 비트 수는 스윙보다 **임계값 높이**가 정한다
    # (실측 순위상관 +0.98 vs +0.49, experiments/swing_sensitivity.py). 임계값이
    # 낮으면 엔벨로프 분포의 밀집 구간에 앉아 작은 이동에도 많은 프레임이 경계를
    # 넘고, 높으면 꼬리에 앉아 주변이 성기다. bd_base 는 네 채널이 0.003~0.009 에
    # 앉아 전체 뒤집힘의 67% 를 만든다.
    #
    # 오프셋을 주입해 강건해지라고 하는 것과 다르다 -- 그건 없는 변동성까지 대비하게
    # 해서 10.7pp 를 먹었다. 이건 **탐색 공간을 만들 수 있는 곳으로 제한**한다.
