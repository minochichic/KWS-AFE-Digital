...
class MetricTests(unittest.TestCase):
    ...
    def test_voltage_display_uses_millivolts(self) -> None:
        """A 0.9174 V internal value must display as two-decimal millivolts."""

        self.assertEqual(format_voltage_mv(0.9174), "917.40 mV")
        self.assertEqual(format_voltage_mv(0.91783245), "917.83 mV")
    ...
    def test_voltage_margin_is_vref_minus_vdet(self) -> None:
        ...
        self.assertEqual(format_margin_mv(margin), "+6.58 mV")
        self.assertEqual(format_margin_mv(-margin), "-6.58 mV")
...
class TransientTests(unittest.TestCase):
    ...
    def test_transient_raw_contains_only_four_requested_analog_voltages(
        ...
        """The viewer reads v_in, v_filt, v_env, and v_thr without v_comp logic."""
...
class TransientDisplayTests(unittest.TestCase):
    ...
    def test_transient_renderer_uses_numeric_ranges_without_span_selector(self) -> None:
        ...
        self.assertIn("v_in [mV]", render_source)
...
class UiInvariantTests(unittest.TestCase):
    ...
    def test_five_workflow_tabs_have_requested_order_and_dc_default(self) -> None:
        """DC, AC, Detector, transient, and PWL are ordered top-level tabs."""
        ...
        self.assertLess(
            source.index('self.workflow_notebook.add(self.ac_page'),
            source.index('self.workflow_notebook.add(self.detector_page'),
        )
        self.assertLess(
            source.index('self.workflow_notebook.add(self.detector_page'),
    ...
    def test_vthr_and_margin_are_mutually_derived_editors(self) -> None:
        """Either divider control may be typed; the other one follows."""

        source = inspect.getsource(SweeperGUI._build_op_detail_tables)
        self.assertIn("목표 V_margin [mV]", source)
        self.assertIn("Vthr [mV]", source)
        self.assertIn("_preview_resistors_from_margin", source)
        self.assertIn("_preview_margin_from_vthr", source)
        # Neither field may be display-only now that both are inputs.
        self.assertNotIn('state="readonly"', source.split("U3 opamp")[0])
        commit = inspect.getsource(SweeperGUI._commit_component_editor)
        self.assertIn("requested_vthr_mv", commit)
        self.assertIn("requested_margin_mv", commit)
        self.assertIn("detector_opamp", commit)
        ...
        paths = tuple(
            sorted(
                (
                    ...
                    *tuple((project_root / "sweeper").rglob("*.py")),
                    *tuple((project_root / "tests").glob("*.py")),
                ),
